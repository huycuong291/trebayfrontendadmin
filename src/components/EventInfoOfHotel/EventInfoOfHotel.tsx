...
import { addHolidayForHotel, updateHoliday } from "@/api/event";
...
export default function EventInfoOfHotel({ objectId, type, holiday, setUpdated, updated }: { objectId: string, type: string, holiday?: HolidayProps, setUpdated: React.Dispatch<React.SetStateAction<boolean>>, updated: boolean }) {

  const handleAddHoliday = async () => {
    if (form.getInputProps('fee').value !== 0 || form.getInputProps('promotion').value !== 0) {
      const holidayData = {
        hotelID: String(form.getInputProps('hotelID').value),
      ...
      const responseData = await addHolidayForHotel(holidayData);
      if (responseData) {
      ...
      >
        ...
        <Group position="right" mt="md">
          <Button variant="outline" onClick={() => {
            form.reset();
            form.setFieldValue('hotelID', objectId);
          }}>
            Reset
          </Button>
