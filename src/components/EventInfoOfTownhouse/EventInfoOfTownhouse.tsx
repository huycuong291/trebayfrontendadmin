...
import { addHolidayForTownhouse, updateHoliday } from "@/api/event";
...
export default function EventInfoOfTownhouse({ objectId, type, holiday, setUpdated, updated }: { objectId: string, type: string, holiday?: HolidayProps, setUpdated: React.Dispatch<React.SetStateAction<boolean>>, updated: boolean }) {

  const handleAddHoliday = async () => {
    if (form.getInputProps('fee').value !== 0 || form.getInputProps('promotion').value !== 0) {
      const holidayData = {
        townhouseID: objectId,
      ...
      const responseData = await addHolidayForTownhouse(holidayData);
      if (responseData) {
      ...
      >
        ...
        <Group position="right" mt="md">
          <Button variant="outline" onClick={() => {
            form.reset();
            form.setFieldValue('townHouseID', objectId);
          }}>
            Reset
          </Button>
